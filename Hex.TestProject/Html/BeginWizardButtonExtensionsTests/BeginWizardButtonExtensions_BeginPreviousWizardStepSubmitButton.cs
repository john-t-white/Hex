...
{
	[TestClass]
	public class BeginWizardButtonExtensions_BeginPreviousWizardStepSubmitButton
	{
		[TestMethod]
		public void ReturnsCorrectly()
		{
			...
			var result = htmlHelper.BeginPreviousWizardStepSubmitButton();

			string expectedResult = string.Format( "<button name=\"{0}\" type=\"submit\" value=\"Previous\">", Constants.WIZARD_PREVIOUS_BUTTON_NAME );
		...
		public void WithNoPreviousStepReturnsCorrectly()
		{
			...
			string expectedResult = string.Format( "<button disabled=\"disabled\" name=\"{0}\" type=\"submit\" value=\"Previous\">", Constants.WIZARD_PREVIOUS_BUTTON_NAME );
			Assert.AreEqual( expectedResult, htmlHelper.ViewContext.Writer.ToString() );
		}
		...
		public void WithHtmlAttributesObjectReturnsCorrectly()
		{
			...
			var result = htmlHelper.BeginPreviousWizardStepSubmitButton( htmlAttributes );

			string expectedResult = string.Format( "<button AttributeName=\"{0}\" name=\"{1}\" type=\"submit\" value=\"Previous\">", attributeValue, Constants.WIZARD_PREVIOUS_BUTTON_NAME );
			Assert.AreEqual( expectedResult, htmlHelper.ViewContext.Writer.ToString() );
		}
		...
		public void WithHtmlAttributesDictionaryReturnsCorrectly()
		{
			...
			string expectedResult = string.Format( "<button {0}=\"{1}\" name=\"{2}\" type=\"submit\" value=\"Previous\">", attributeName, attributeValue, Constants.WIZARD_PREVIOUS_BUTTON_NAME );
			Assert.AreEqual( expectedResult, htmlHelper.ViewContext.Writer.ToString() );
		}
		...
		public void WithAttributeExpressionReturnsCorrectly()
		{
			...
			var result = htmlHelper.BeginPreviousWizardStepSubmitButton( x => x.Attribute( attributeName, attributeValue ) );
