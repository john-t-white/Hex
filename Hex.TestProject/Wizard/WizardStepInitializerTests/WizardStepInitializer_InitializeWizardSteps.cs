...
namespace Hex.TestProject.Wizard.WizardStepInitializerTests
{
	...
	public class WizardStepInitializer_InitializeWizardSteps
	{
		...
		{
			RequestContext requestContext = new RequestContext();
			WizardController wizardController = ( WizardController )Activator.CreateInstance( wizardControllerType );
			ControllerContext controllerContext = new ControllerContext( requestContext, wizardController );

			return wizardController.ActionInvoker.GetWizardActions( controllerContext );
		}
	}
}
